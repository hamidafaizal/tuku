...
export default function DatabaseBarang() {
  ...
  const handleSelectItem = (id) => setSelectedItems(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, i]);
  const handleSelectAll = () => setSelectedItems(selectedItems.length === filteredItems.length ? [] : filteredItems.map(i => i.id));
  ...
  return (
    <>
      <div className="space-y-4">
        {/* Header dan Toolbar */}
        {/* ... (UI tidak berubah, hanya handler yang diperbarui) ... */}
        {isBulkDeleteMode ? (
        ...
        {(isOffline || isSyncing) && (
          <div className={`border-l-4 p-4 rounded-r-lg flex items-center gap-3 ${isOffline ? 'bg-amber-50 border-amber-400' : 'bg-sky-50 border-sky-400'}`}>
